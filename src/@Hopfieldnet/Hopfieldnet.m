classdef Hopfieldnet < handle
  ...
  methods
    ...
    function train(obj, trainingdata, trainingrate, iterations)
      trainingsize = size(trainingdata);
      if ~isequal(trainingsize(1:2), obj.structure)
        disp('wrong trainingdata structure')
        return
      end
      weights = zeros(length(obj.neurons));
      for i = 1:length(obj.neurons)
        weights(i,:) = obj.neurons(i).weight; 
      end
      % flatten the trainingdata
      trainingdata = reshape(trainingdata, ...
        [length(obj.neurons), trainingsize(3)]).';
      for i = 1:iterations
        % update the weights with a random training example
        data = trainingdata(ceil(rand() * trainingsize(3)), :);
        delta = data.' * data;
        weights = weights + trainingrate * delta;
      end
      % set the weights
      for i = 1:length(obj.neurons)
        obj.neurons(i).weight = weights(i,:);
      end
    end
